...
const CategoryView = ({ navigation, route }) => {
    ...
    const { fetchCategory, calculateCategoryDistribution, categories, transactions, getAll } = React.useContext(UserContext);
    ...
    const renderHeader = () => {
        return (
            <View>
                ...
                <View style={{ backgroundColor: 'white', paddingTop: scale(15), paddingHorizontal: scale(20), paddingBottom: scale(5), borderTopRightRadius: scale(15), borderTopLeftRadius: scale(15), gap: scale(7) }}>
    ...
    React.useEffect(() => {
        ...
        var data = calculateCategoryDistribution(category_id);
    ...
    return (
        <CustomFlatListView
            ...
            data={myTransactions?.filter((transaction) => transaction?.name?.toUpperCase()?.includes(filter?.toUpperCase())).sort((a, b) => handleSort(a, b) ? -1 : 1)}
