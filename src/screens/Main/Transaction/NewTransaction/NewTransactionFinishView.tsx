...
const NewTransactionFinishView = ({ navigation, route }: { navigation: any, route: any }) => {
    ...
    const { createTransaction, updateTransaction, fetchTransaction, fetchCategory } = React.useContext(UserContext);
    ...
    return (
        <GestureHandlerRootView style={{ flex: 1 }}>
            ...
            <CustomView backgroundColor={backgroundColor} style={{ flex: 1, justifyContent: 'space-between' }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
...

                    {
                        loading
                            ...
                            :
                            <TouchableOpacity
                                onPress={() => { name != '' && handleComplete() }}
                                disabled={name == ''}
                                style={{ width: scale(35), alignItems: "flex-end" }}
                            >
                                <Ionicons name="checkmark-done-sharp" size={scale(22)} color={name == '' ? 'gray' : green} />
                            </TouchableOpacity>
                    }
                ...
                <CustomTextInput border={undefined} value={name} onChangeText={setName} placeholder={"Enter transaction name"} loading={loading} />
                ...
                >

                    {
                        images != ''
                            ?
                            <Image source={images} style={{ height: '100%', flexDirection: 'row', padding: scale(15), justifyContent: 'space-between' }}>
                ...
                <View style={{ backgroundColor: 'white', borderRadius: scale(5), height: scale(70), justifyContent: 'space-between', flexDirection: 'row', padding: scale(15), alignItems: 'center' }}>
            ...
            >
                <CameraView bottomSheetRef={bottomSheetRef} setPicture={setImages} showCamera={showCamera} />
            </BottomSheet>
